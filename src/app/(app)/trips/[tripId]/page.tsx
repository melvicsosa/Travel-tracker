...
import type { Activity, Traveler, Trip, TripInvite, TripMemberWithProfile } from "@/lib/database.types";
...
export default async function TripPage({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  const [tripRes, travelersRes, activitiesRes, membersRes, invitesRes] = await Promise.all([
    ...
    supabase.from("trip_invites").select("*").eq("trip_id", tripId).order("created_at"),
  ]);
  ...
  return (
    <TripPlanner
      ...
      initialInvites={(invitesRes.data ?? []) as TripInvite[]}
